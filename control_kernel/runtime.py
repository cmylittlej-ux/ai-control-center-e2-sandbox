...
SCHEMA_GENERATION_COMMAND = ("codex", "app-server", "generate-json-schema", "--out")
...
class RuntimePin:
    ...
    def from_json(cls, path: Path) -> "RuntimePin":
        ...
        if not isinstance(version, str) or not isinstance(schema_path, str) or not isinstance(schema_sha256, str):
            raise UnsupportedRuntime("runtime pin is incomplete; refusing to start")
        return cls(version, Path(schema_path), schema_sha256, bool(data.get("fail_closed", True)))

    def validate(self, reported_version: str) -> None:
